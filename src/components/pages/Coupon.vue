<template>
  ...
  <Loading :active.sync="isLoading"></Loading>
    <div class="text-right mt-5">
      <button class="btn btn-primary  mt-5" @click="openCouponModal(true)">
        建立新的優惠券
      </button>
    </div>
    ...
    <Pagination :childPagination="pagination" @changeCurrentPage="getCoupons"></Pagination>
...
export default {
  ...
  watch: {
    due_date() {
      ...
      vm.tempCoupon.due_date = timestamp;// isNew === true
    },
  },
  methods: {
    openCouponModal(isNew, item) {
      ...
      if (vm.isNew) {
        ...
        vm.due_date = dateAndTime[0];// isNew === false
      }
    },
    getCoupons() {
      const vm = this;
